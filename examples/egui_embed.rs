...
//!
//! Stacks figgy::Renderer on top of eframe (winit + egui_wgpu) and draws
//! sine / RC / cross-section charts via paint callbacks.
//!
//! Flow:
//!
//! 1. Get device/queue/format from eframe's `wgpu_render_state`.
//! 2. On first update, build `figgy::Renderer`, register columns, set up
...
//! 5. Callback `paint`: a single `figgy::Renderer::paint(pass, items)` call.
//!
//! Run with:
...
use figgy::config::{AxisOptions, AxisScale, Config, GridOptions, LegendEntryKind};
        ...
        egui::CentralPanel::default()
            ...
            ui.columns(3, |cols| {
                for (i, col_ui) in cols.iter_mut().enumerate() {
                    ...
                    let (rect, _resp) = col_ui.allocate_exact_size(avail, egui::Sense::hover());
